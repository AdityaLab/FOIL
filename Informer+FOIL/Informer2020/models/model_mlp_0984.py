...
class Informer(nn.Module):
    def __init__(self, enc_in, dec_in, c_out, seq_len, label_len, out_len, 
                ...
                device=torch.device('cuda:0'),penalty_anneal_iters=5,sigma=1.0,alpha=0.5,lam=0.1,hard_sum = 1.0,hidden_dim=1024,emb_dim=512 ):
        ...
        self.mlp_inv = self.make_mlp(d_model, hidden_dim, emb_dim, c_out)
        self.mlp_var = self.make_mlp(d_model, hidden_dim, emb_dim, c_out)
    def make_mlp(self, input_dim, hidden_dim, output_dim, c_out):
        layers = []
        layers.append(nn.Linear(input_dim, hidden_dim))
        layers.append(nn.ReLU())
        layers.append(nn.Linear(hidden_dim, output_dim))
        layers.append(nn.ReLU())
        layers.append(nn.Linear(output_dim, c_out))
        return nn.Sequential(*layers)
    def renew(self):
        self.ln = nn.Linear(self.d_model, self.c_out, bias=True).to(self.device)
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, batch_y,step,scale,flag,
        ...
        inv_emb=self.mlp_inv(dec_out_OT)
        #var_emb=self.mlp_var(dec_out_OT)

        #相当于输出是label len+pred len
        #final_out=self.ln(inv_emb)
        final_out=inv_emb 
