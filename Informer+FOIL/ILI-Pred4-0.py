...
args.root_path = '../dataset/illness/' # root path of data file
args.data_path = 'national_illness.csv' # data file
...
args.seq_len = 36 # input sequence length of Informer encoder
args.label_len = 18 # start token length of Informer decoder
...
print(args)
devices = torch.device(f"cuda:{args.devices}") 
...
if stage1==1:
    ...
    for i in range(1):
        ...
        model_pre.to('cpu')
        env_softmax_np = env_softmax.cpu().detach().numpy()
        env_softmax_2=Smooth_Labels(env_softmax_np,neighborhood=1)
        print(env_softmax_2.shape)
        softmax_filename = '1-ILIPRE_NEW_'+str(args.env_num)+'class.csv' 
        pd.DataFrame(env_softmax_2).to_csv(softmax_filename, index=False)  
        print(f'Saved env_softmax for iteration {ii} as {softmax_filename}')
...
if stage2==1:
    ...
    w_env_path='1-ILIPRE_NEW_'+str(args.env_num)+'class.csv' 
